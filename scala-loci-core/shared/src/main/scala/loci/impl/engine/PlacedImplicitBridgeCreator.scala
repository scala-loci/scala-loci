...
class PlacedImplicitBridgeCreator[C <: Context](protected val c: C) {
  ...
  def createPlacedImplicitBridges: List[Tree] = {
    val lociType = (c.mirror staticPackage "_root_.loci").moduleClass.asType
    ...
    def isInLociPackage(tpe: Type): Boolean = {
      def isInLociPackage(symbol: Symbol): Boolean =
        symbol == lociType ||
        (symbol.owner != NoSymbol && isInLociPackage(symbol.owner))

      tpe exists { tpe => isInLociPackage(tpe.typeSymbol) }
    }
    ...
    findImportedImplicitConversions flatMap {
      case ImportedImplicitConversion(method, importing) =>
        ...
        val extraParamDef =
          q"${Flag.PARAM} val $ev: _root_.loci.LocalValueTypes[$T, $U]"
        ...
        val result =
          q"""${Flag.SYNTHETIC | Flag.ARTIFACT} implicit def $name
                ...
                : $returnType = _root_.loci.`#macro`"""

        val exclude =
          hasUnboundedParam ||
          isUnbounded(paramType) ||
          isUnbounded(returnType) ||
          isInLociPackage(paramType) ||
          isInLociPackage(returnType) ||
          (typeParams exists { symbol =>
            isInLociPackage(symbol typeSignatureIn importing)
          })
