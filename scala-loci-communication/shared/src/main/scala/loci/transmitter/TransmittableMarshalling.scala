...
object transmittableMarshalling {
  ...
  implicit class MarshallableOps[T, S, R](
    ...
    def unmarshal(marshalled: MessageBuffer, abstraction: AbstractionRef): Try[R] =
      currentAbstraction.withValue(Some((abstraction, 0))) {
  ...
  private class EndpointImpl[T, U](val abstraction: AbstractionRef,
    ...
    abstraction.channel.receive notify
      withValue(0) { (value: MessageBuffer, turn) =>
        unmarshall(value,
    ...
    private def unmarshall(value: MessageBuffer, abstraction: AbstractionRef) =
      currentAbstraction.withValue(Some((abstraction, 0))) {
