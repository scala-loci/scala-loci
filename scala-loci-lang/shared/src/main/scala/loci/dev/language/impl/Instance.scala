  ...
  def start(instance: Tree): Tree = {
    ...
    expr match {
      case q"new { ..$earlydefs } with $inst[$tpt](...$exprss) { $self => ..$body }"
        ...
        // extract context and connection setup from arguments
        val (context, connect) = exprs match {
          case Nil =>
            q"${termNames.ROOTPKG}.loci.contexts.Queued.create" ->
              q"${termNames.ROOTPKG}.scala.collection.immutable.Map.empty"

          case context :: Nil if context.tpe <:< types.executionContext =>
            context ->
              q"${termNames.ROOTPKG}.scala.collection.immutable.Map.empty"

          case connect :: Nil =>
            q"${termNames.ROOTPKG}.loci.contexts.Queued.create" ->
              q"$connect.setup($prefix.$signature, $prefix.$ties.keys.flatMap(_.bases).toList)"

          case context :: connect :: _ =>
            context ->
              q"$connect.setup($prefix.$signature, $prefix.$ties.keys.flatMap(_.bases).toList)"
        }
        ...
        val main =
          if ((prefix.tpe member TermName("main")).alternatives exists { symbol =>
            ...
            q"${termNames.ROOTPKG}.scala.Some($$loci$$instance.main _)"
          else
            q"${termNames.ROOTPKG}.scala.None"

        val system = q"""${Flag.SYNTHETIC} protected def $$loci$$sys$$create =
          new ${types.system}(this, context, connections, connected, connecting)"""

        val tree = transformer transform q"""
          ..$namedArgs
          ${termNames.ROOTPKG}.loci.dev.runtime.Instance.start(
            $prefix.$signature,
            $prefix.$ties,
            $context,
            $connect,
            (context, connections, connected, connecting) => {
              ...
              val $$loci$$instance =
                ...
                $$loci$$instance.$$loci$$sys.start($main)
                $$loci$$instance.$$loci$$sys
            })"""
