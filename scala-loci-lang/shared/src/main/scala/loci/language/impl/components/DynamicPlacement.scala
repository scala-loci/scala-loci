    ...
    records process {
      case Initialized(tree) =>
        ...
        def simplifyRuleType(ruleType: Type, pos: Position): Type = {
          ruleType match {
            case ruleType if isParameterlessRule(ruleType) || isRuleFunctionOnRemoteSeq(ruleType) => ruleType
